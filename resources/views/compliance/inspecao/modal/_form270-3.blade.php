...
<div id="aprimoramento">
    ...
    Conciliação SMB x BDF – dados “Não Conciliados”, em {{ date( 'd/m/Y' , strtotime($dtfim))}},
    ...
    @if(isset($smb_bdf_naoconciliados)&&(!empty($smb_bdf_naoconciliados)))
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan 4>{{($total > 0 ? ' Valor em Falta' : ' Valor em Sobra')}}</td>
...
<div id="aprimoramento">
    <span class="lever" text-align="rigth">
        ...
        "Contabilização", Conciliação SMB x BDF – dados “Não Conciliados”, em
         {{ date( 'd/m/Y' , strtotime($dtfim))}}, referente ao período de {{ date( 'd/m/Y' , strtotime($dtini))}} a {{ date( 'd/m/Y' , strtotime($dtfim))}}, verificou-se a inexistência de divergências.
    </span>
</div>
@endif

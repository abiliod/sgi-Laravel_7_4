...
@elseif ($res >  0)
    ...
    <div id="historico">
        ...
        <span class="lever rigth">
                Foram avaliados {{$amostra}} objetos relativo ao período de {{substr($dtini,8,2)}}/{{substr($dtini,5,2)}}/{{substr($dtini,0,4)}} até {{substr($dtfim,8,2)}}/{{substr($dtfim,5,2)}}/{{substr($dtfim,0,4)}}, que representavam 100 por cento do total da amostra para o período mencionado, sendo que em {{$percentagem_falhas}} por cento constam inconsistências.
        </span>
    </div>
    <div id="historico1"></div>
@endif
